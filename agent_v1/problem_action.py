from agent_v1.prompt import INTENT_ANALYSIS_PROMPT,KEYWORDS_ANALYSIS_PROMPT
from metagpt.actions import Action
from agent_v1.common_util import is_json
from typing import Optional, List, Dict
from metagpt.tools.search_engine import SearchEngine
from pydantic import Field, model_validator
from metagpt.schema import Message
from metagpt.logs import logger
from agent_v1.tool_details import TOOL_DETEAIL
from metagpt.utils.common import OutputParser
from agent_v1.tool_util_details import call_tool_by_condition
...
class RagAction(Action):
    ...
    async def run(self, instruction: list[Message], max_tasks,system_text=KNOWLEDGE_BASE_SYSTEM_PROMPT) -> str:
        if self.search_engine is None:
...
class FaultContentAction(Action):
    ...
    async def run(self, context:List[Message]) -> str:
        ...
        for first_dir in context:
            global global_intent_info
            sub_dirs = context[first_dir]
            tool_results = global_intent_info['tool_result']
            prompt = FAULT_CONTENT_COMMON_PROMPT_V1.format(
                topic=global_instruction_keywords,
                language=self.language,
                directory=first_dir,
                subdirectory=sub_dirs,
                tool_results=tool_results,
                steps=global_intent_info['steps']
            )
